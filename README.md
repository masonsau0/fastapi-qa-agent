...
A tool-using agent that answers questions about the [FastAPI](https://github.com/fastapi/fastapi) codebase. Ask it "how does dependency injection work?" or "why did this file change recently?" and it goes and figures it out — searches the code, searches the docs, reads specific files, looks at git history.
...
Spoiler from my dev runs: the agent wins on questions that need following a thread (search → read more → check git), and ties on simple lookup questions. So the answer to "should I always use an agent" is no.
...
- A 50-question benchmark mined from real FastAPI GitHub issues + their fix PRs. The mining is automated, the curation is manual (I read each one and verified the answer). See `src/eval/mine_benchmark.py`.
...
You need Python 3.10+, git, and an Anthropic API key. Optional but recommended: a GitHub Personal Access Token (raises the API rate limit from 60 to 5000 req/hr — matters for the benchmark miner).
...
Before anything else, **set a spending cap on the Anthropic console.** $15/month is plenty for this project (the full eval run is $1–3). See `SECURITY.md` for the why.
...
The clone is shallow (depth 200 commits) so it's small. The index build takes ~2 minutes on a laptop — most of that is computing embeddings for ~5K code chunks.
...
- Set `curated_answer_keywords` to 3–6 short strings that any correct answer should contain.
...
This part is real work — budget 2–3 hours. The quality of the eval is entirely determined by how careful you are here.
